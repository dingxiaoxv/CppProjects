...
struct LeAdvertisingService {
  ...
  const uint8_t type = 0x03; // Complete list of 16-bit Service Class UUIDs
  uint8_t uuid[2];
};
...
struct LeAdvertisingLocalName {
  const uint8_t length = 0x17;
  const uint8_t type = 0x09; // Complete local name
  uint8_t name[22];
};
...
class HciHelper {
  HciHelper();
public:
  static HciHelper& instance();
  ~HciHelper();
  bool valid() const { return fd_ >= 0; }
  HCI_VERSION getHciVersion();
  void setLeAdvertisingData(uint16_t service, const char* data, const char* mac = nullptr);
  void reEnableAdvertsing();
  void setLeAdvertisingDataExt(uint16_t service, const char* data);
  std::string getMacAddress();
  int getPeerCount();
  void leReadRemoteFeatures();
  void leReadLocalFeatures();
  int8_t getRssi();
  void disconnect();
  void enableLeScan();
  void filterLeMetaEvent();
  int fd() { return fd_; }
